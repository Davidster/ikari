...
impl InstancedMeshComponent {
    pub fn new(
        ...
        // let diffuse_texture_mag = match diffuse_texture {
        //     Some(diffuse_texture) => MaybeDefaultedRef::Passed(diffuse_texture),
        //     None => MaybeDefaultedRef::Defaulted(Texture::from_color(
        //         device,
        //         queue,
        //         [255, 255, 255, 255],
        //     )?),
        // };
        // let diffuse_texture = match &diffuse_texture_mag {
        //     MaybeDefaultedRef::Passed(texture) => texture,
        //     MaybeDefaultedRef::Defaulted(texture) => texture,
        // };
        ...
        let normal_map_mag = match normal_map {
            Some(normal_map) => MaybeDefaultedRef::Passed(normal_map),
            None => MaybeDefaultedRef::Defaulted(Texture::flat_normal_map(device, queue)?),
        };
        let normal_map = match &normal_map_mag {
            MaybeDefaultedRef::Passed(texture) => texture,
            MaybeDefaultedRef::Defaulted(texture) => texture,
        };
