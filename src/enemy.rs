...
pub struct Enemy {
    ...
    walking_animation_index: usize,
    navigation_state: EnemyNavigationState,
}

pub enum EnemyNavigationState {
    Idle,
    Navigating { destination: Vector2<f32> },
}
...
impl Enemy {
    pub fn new(
        ...
        if let Some(idle_animation) = scene.animations.get_mut(idle_animation_index) {
            idle_animation.speed = 1.0;
            idle_animation.state.is_playing = true;
            idle_animation.state.loop_type = LoopType::PingPong;
        }
        ...
        let mut result = Self {
            ...
            walking_animation_index,
            navigation_state: EnemyNavigationState::Idle,
        };
